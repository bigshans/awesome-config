...
run_once({"bash", "/opt/touchpad.sh"})
os.execute("xautolock -time 10 -locker 'i3lock-fancy -gpf Comic-Sans-MS' &")
os.execute("volumeicon &")
run_once({"caffeine-indicator"})
run_once({"bash", "systemctl --user start emacs"})
...
globalkeys = my_table.join(
    ...
    awful.key({ modkey,  }, "q", function () os.execute("/opt/powermenu.sh") end,
...
awful.rules.rules = {
    ...
    { rule = { class = "netease-cloud-music" },
          properties = {floating = true, tag = awful.util.tagnames[6]}},
    { rule = { class = "Telegram" },
